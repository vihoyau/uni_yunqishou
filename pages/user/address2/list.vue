...
<template>
	<view class="address-wrap">
		<view class="address-list" v-for="address in addressList" :key="address.id" @tap="useAddress(address)">
			...
			<button class="u-reset-button set-btn" @tap.stop="jump('/pages/user/address2/edit', { id: address.id })">编辑</button>
		</view>
		...
		<view class="foot_box-wrap safe-area-inset-bottom">
			<view class="foot_box u-flex u-row-between safe-area-inset-bottom">
				...
				<button class="u-reset-button add-btn u-m-20" @tap="jump('/pages/user/address2/edit')">添加新的收货地址</button>
			</view>
		</view>
...
export default {
	...
	methods: {
		...
		useAddress(addressData) {
			uni.$emit('SELECT_ADDRESS2', { addressData: JSON.stringify(addressData) });
			this.$Router.back();
		},
		// 路由跳转
		jump(path, parmas) {
		...
		getWXaddress() {
			...
			authState &&
				uni.chooseAddress({
					success: res => {
						this.jump('/pages/user/address2/edit', { addressData: res });
					},
					fail: err => {
		...
		getAddressList() {
			this.$http('address.list2').then(res => {
				if (res.code === 1) {
					this.addressList = res.data;
					!this.addressList.length && uni.$emit('SELECT_ADDRESS2', { addressData: null });
				}
			});
		}
	}
