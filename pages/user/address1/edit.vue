	...
	<view class="address-wrap">
		...
		<!-- 	<view class="location-item u-flex u-row-between u-p-20 u-m-b-20" @tap="getLocation">
			...
			<view class="address-box">
				...
				<u-form-item :labelStyle="labelStyle" 　 label-width="150" label-position="left" label="寄件人:" prop="consignee">
					<u-input placeholder="请填写寄件人/店名" v-model="model.consignee" type="text"></u-input>
				</u-form-item>
				<u-form-item :labelStyle="labelStyle" 　 label-width="150" label-position="left" label="手机号:" prop="phone">
					<u-input placeholder="请输入寄件人手机号" v-model="model.phone" type="number"></u-input>
				</u-form-item>
				<u-form-item :labelStyle="labelStyle" label-width="150" label-position="left" label="取货定位:" prop="area_text">
					<u-input type="select" v-model="model.area_text" :select-open="showSelect" placeholder="取货定位" @tap="getLocation"></u-input>
				</u-form-item>
			...
			<view class="foot_box u-p-30 u-m-t-50 u-flex u-row-around">
				<button v-show="addressId" class=" u-m-20 u-reset-button delete-btn u-flex-1" v-if="addressId" @tap="deleteAddress">删除该地址</button>
				<button class=" u-m-20 u-reset-button save-btn u-flex-1"  v-else @tap="submit">保存取件地址</button>
			</view>
		</u-form>
...
export default {
	...
	onLoad() {
		...
		uni.setNavigationBarTitle({
			title: this.addressId ? '编辑收货地址' : '填写取件信息'
		});
	...
	methods: {
		...
		// 确认省市区
		regionConfirm(e) {
			console.log("确认省市区"+JSON.stringify(e))
		...
		async getLocationInfo() {
			...
			if (res.data.status === '1') {
				console.log("地址数据"+JSON.stringify(res.data))
		...
		getAddressInfo() {
			...
			this.$http('address.info', {
				...
				if (res.code === 1) {
					...
					that.model.area_text = addressData.area_text;
